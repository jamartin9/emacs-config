...
The dependencies are: emacs >= 29 (with features gnutls (melpa), sqlite (forge/roam)), git (magit), guix (guix.el), pass/gpg (password-store/pass-otp/authinfo), aspell (flyspell), mpv (emms), notification-daemon (erc), osc terminal emulator (xterm.el), curl (osm), wget (newsticker), transmission and a shell.
