...
(use-package! pass
  :general (global-map "C-c o p" #'pass)
  :commands pass)
...
(use-package! newsticker ; built-in
  ...
  :general
  (newsticker-treeview-item-mode-map "d" #'jam/newsticker-download)
  (global-map "C-c o n" #'newsticker-treeview)
  (global-map "C-c t n" #'newsticker-stop)
  :init (setq   newsticker-frontend 'newsticker-treeview
                newsticker-dir (concat (concat user-emacs-directory ".local/cache/") "newsticker")
...
(use-package! switch-window
  :general (window-prefix-map "w" #'switch-window)
...
(use-package! vterm ; maybe bind C-v to vterm-yank
  :general (global-map "C-c o t" #'jam/vterm)
  :commands (vterm-mode vterm)
  :config (setq vterm-kill-buffer-on-exit t
                vterm-max-scrollback 5000))

(use-package! undo-tree
  :custom (undo-tree-history-directory-alist `(("." . ,(concat (concat user-emacs-directory ".local/cache/") "undo-tree-hist/"))))
...
(use-package! magit
  ...
  :general (global-map "C-c v s" #'magit-status)
  :init
  (setq magit-auto-revert-mode nil)
  (setq transient-levels-file  (concat (concat user-emacs-directory ".local/etc/") "transient/levels")
        transient-values-file  (concat (concat user-emacs-directory ".local/etc/") "transient/values")
        transient-history-file (concat (concat user-emacs-directory ".local/etc/") "transient/history"))
  :config
  ...
  :general
  (transient-map [escape] #'transient-quit-one)
  (magit-mode-map "q" #'kill-buffer))

(use-package! magit-todos
  :after magit
  :config
  (setq magit-todos-keyword-suffix "\\(?:([^)]+)\\)?:?") ; make colon optional
  :general (magit-todos-section-map "j" nil))
...
(use-package! forge
  ...
  :init
  (setq forge-database-file (concat (concat user-emacs-directory ".local/etc/") "forge/forge-database.sqlite"))
  (setq forge-add-default-bindings t))

(use-package! code-review
  :after magit
  :init
  (setq code-review-db-database-file (concat (concat user-emacs-directory ".local/etc/") "code-review/code-review-db-file.sqlite")
        code-review-log-file (concat (concat user-emacs-directory ".local/etc/") "code-review/code-review-error.log")
        code-review-download-dir (concat (concat user-emacs-directory ".local/etc/") "code-review/")
...
(use-package! hideshow ; built-in
  :general (global-map "C-c t z" #'hs-minor-mode)
           (global-map "C-c c z" #'hs-hide-block)
           (global-map "C-c c v" #'hs-toggle-hiding)
...
(use-package! org ; built-in
  :commands org-mode ; (setq org-timer-default-timer 60)
...
(use-package! org-roam
  :general (global-map "C-c n r" #'org-roam-node-find)
  ...
  :init (setq org-roam-directory (concat (expand-file-name doom-user-dir) (file-name-as-directory "org") (file-name-as-directory "roam")) ; TODO sub doom-user-dir for emacs-user-directory when dropping doom
              org-directory org-roam-directory
              org-id-locations-file (expand-file-name ".orgids" org-directory) ; org-id-update-id-locations org-roam-db-sync
              org-return-follows-link t
              org-persist-directory (concat (concat user-emacs-directory ".local/cache/") "org-persist/")
...
(use-package! fd-dired
  :after dired
  :config (define-key dired-mode-map [remap find-dired] #'fd-dired)
          (define-key dired-mode-map [remap find-grep-dired] #'fd-grep-dired)
          (define-key dired-mode-map [remap find-name-dired] #'fd-name-dired)
...
(use-package! geiser
  ...
  (setq geiser-repl-per-project-p t
        ...
        geiser-repl-history-filename (concat (concat user-emacs-directory ".local/cache/") "geiser-history")))
...
(use-package! erc ; built-in
  ...
  :config (setq erc-rename-buffers t
                ...
                erc-log-channels-directory (concat (concat user-emacs-directory ".local/etc/") "erc")
...
(use-package! eshell ; built-in
  ...
  :commands (eshell)
  :general (global-map "C-c o e" #'eshell)
  :config
  (require 'em-smart)
...
(use-package! emacs ; built-in
  :config (setq doom-theme 'modus-vivendi
                doom-font (font-spec :size 12 :family "monospace"); 8
                +doom-dashboard-ascii-banner-fn #'jam/utf-banner
                +doom-dashboard-banner-padding '(0)
                +doom-dashboard--width 96; 188
                user-full-name "Justin Martin"
                user-mail-address "jaming@protonmail.com"
                auth-sources (list (concat (concat user-emacs-directory ".local/cache/") "authinfo.gpg") "~/.authinfo.gpg") ; auth
                ...
                ;gc-cons-threshold (* 128 1024 1024); gcmh managed and most-positive-fixnum on early-init
                package-enable-at-startup nil ; packages
                native-comp-deferred-compilation t ; native comp
                native-comp-async-report-warnings-errors nil
                native-compile-target-directory (concat (concat user-emacs-directory ".local/cache/") "eln/")
                native-comp-eln-load-path (add-to-list 'native-comp-eln-load-path (concat (concat user-emacs-directory ".local/cache/") "eln/"))
            ...
            (setq-default indent-tabs-mode nil
                ...
                word-wrap t)
            (add-hook 'tty-setup-hook #'xterm-mouse-mode); use builtins instead of general-after-tty/general-after-gui/general-add-hook/general-setq
            ...
            (blink-cursor-mode -1)
            (define-key ctl-x-map (kbd "C-S-s") #'jam/save-all) ; save all buffers
            (define-key help-map (kbd "D") #'shortdoc) ; cheat sheet
            (define-key emacs-lisp-mode-map (kbd "C-c C-S-f") #'pp-buffer) ; format
            (modus-themes-load-vivendi); hook server-after-make-frame-hook for daemonp?
...
(use-package! gnus ; built-in
  ...
  (setq
   ...
   gnus-dribble-directory (concat (concat user-emacs-directory ".local/cache/") (file-name-as-directory "gnus"))
   gnus-startup-file (concat (concat user-emacs-directory ".local/cache/") (file-name-as-directory "gnus") "newsrc")
   gnus-select-method
          '(nnimap "gmail"
    ...
    (eval-after-load 'gnus-topic
      '(progn
         (setq gnus-message-archive-group '((format-time-string "sent.%Y"))
               gnus-server-alist '(("archive" nnfolder "archive" (nnfolder-directory (concat (concat user-emacs-directory ".local/cache/") "archive"))
                                   (nnfolder-active-file (concat (concat user-emacs-directory ".local/cache/") (file-name-as-directory "archive") "active"))
...
(use-package! osm
  ...
  ;:init (with-eval-after-load 'org (require 'osm-ol))
...
(use-package! multiple-cursors
  ...
  :config (setq mc/list-file (concat (concat user-emacs-directory ".local/etc/") "mc-lists.el")))

(use-package! drag-stuff
  :defer t
  :general ("<M-up>" 'drag-stuff-up)
