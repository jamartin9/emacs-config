...
* sqlite
** notes
- turso rust rewrite is async
  - with wasm https://github.com/tursodatabase/turso/blob/main/bindings/javascript/examples/wasm/index.html MAYBE switch site?
...
onmessage = (e) => {
    ...
    if (typeof globalThis.sqlite3InitModule === 'undefined') { // load sqlite3
        ...
        globalThis.sqlite3InitModule().then((sqlite3)=> {
            sqlite3.installOpfsSAHPoolVfs().then((poolUtil)=> { // use opfs for storage. MAYBE Add weblocks and broadcast api for multi tab
