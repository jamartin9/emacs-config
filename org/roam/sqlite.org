          ...
          if (typeof globalThis.db === 'undefined') { // load sqlite db
              window.sqlite3InitModule().then((sqlite3)=>{
                ...
                /*sqlite3.installOpfsSAHPoolVfs().then((poolUtil)=> {
                    console.log("installing opfs");
                    if poolUtil.getFileCount() == 0 { // MAYBE check name/date with transparent naming
                      ...
                      console.log(poolUtil.getFileNames());
                      //const db = new PoolUtil.OpfsSAHPoolDb('/sqlar.sqlite')
                    }
                }).catch( console.log("Failed to use opfs"); ); */ // maybe fallback or hide search
                  const db = new oo.DB();
                  fetch('sqlar.sqlite').then(response => response.arrayBuffer()).then(arrayBuffer => {
                      const bytes = new Uint8Array(arrayBuffer);
