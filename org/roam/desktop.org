...
- kde
  ...
  - kde6 built in remote desktop support
  - brave pip draw over all windows with 'Window Management' -> 'Window Rules'
    - window class (application) as unimportant and 'brave-browser'
    - no for 'Match whole window class'
    - 'Window types' as 'All selected'
    - 'Window title' as 'Substring match' set to 'Picture in picture'
    - under 'Arrangement & Access' add property for 'Keep above other windows' to 'Force' and 'Yes'
    - 'Detect Window Properties' set to 'Instantly'
- aero (windows)
- quartz (mac)
