...
** guix netbook
#+NAME guix-netbook
#+BEGIN_SRC scheme :tangle netbook.scm :noweb yes
(use-modules
 (gnu)
 (gnu services)
 (gnu services linux)
...
(operating-system
  ...
  (services
   (append (list (service xfce-desktop-service-type)
                 ...
                 (simple-service 'channel-file etc-service-type
                                 (list `("nonguix-pub.scm" ,(local-file "nonguix-pub.scm"))))
                 (simple-service 'jam-home-service home-service-type
                                  `(("jam" ,jam-home)))
                 )
...
#+BEGIN_SRC scheme :tangle no
  ;; nvidia service
  (use-modules (nongnu services nvidia) (nongnu packages nvidia))
  ...
  (define %nvidia-xorg-config ;; nvidia xorg configuration
    ...
    EndSection")
    ;(append %default-kernel-arguments ("modprobe.blacklist=nouveau" "nvidia_drm.modeset=1"))
    ;(xorg-configuration
    ;  (xorg-configuration
    ;    (modules (cons* nvidia-driver ;; nvidia-xorg
    ;        %default-xorg-modules))
    ;    (server (nvidia-transform xorg-server)) ; mesa to nvda graft, graft-mesa to gnome/xfce/kde
	;    (drivers '("nvidia"))
