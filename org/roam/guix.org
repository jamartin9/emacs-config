...
#+BEGIN_SRC scheme :tangle home.scm :noweb yes
  ...
  (define ssh-service ; writes to /etc/dropbear for keys
    (set-fields       ; least-authority-wrapper to contain
  ...
  (home-environment
   (packages (append
            ...
            (map specification->package (list "guile"
                                             ...
                                             "screen"
                                             "tree-sitter-rust" "tree-sitter-yaml" "tree-sitter-python"
                                             "emacs-eat" "emacs-debbugs" "emacs-org-roam" "emacs-guix" "emacs-osm" "emacs-minions" "emacs-transmission" "emacs-undo-tree" "emacs-pass" "emacs-password-store-otp" "emacs-company" "emacs-which-key" "emacs-macrostep-geiser" "emacs-geiser-guile" "emacs-flymake-guile" "emacs-pyvenv" "emacs-code-review" "emacs-dape"
                                             ))))
   (services
    (list
...
** guixsd config
#+NAME: guix-os
#+BEGIN_SRC scheme :tangle guix-os.scm :noweb yes
  (use-modules
   (gnu)
   (gnu services)
   (gnu services linux)
  ...
  (use-package-modules bootloaders certs fonts nvi
                       ...
                       xdisorg ;; SPICE dynamic resizing for xfce
)

  (define my-kernel (corrupt-linux linux-libre-with-bpf ));#:configs '("CONFIG_DEBUG_INFO_BTF=y"))); BUG with menuconfig

  <<guix-os-snips-kicksecure>>
  ...
  %wheel ALL=NOPASSWD: ALL\n"))
    ...
    (packages (append (list nss-certs ; https
                            x-resize ; xfce spice resizing https://gitlab.xfce.org/xfce/xfce4-settings/-/issues/142
             ...
             (remove (lambda (service)
                     ...
                     (modify-services %desktop-services
                       ...
                       (guix-service-type config =>
                                           (guix-configuration
                                            (inherit config); ADD guix-configuration-channels instead of etc file above
...
#+NAME: guix-installer
#+BEGIN_SRC scheme :tangle installer.scm
...
** guix netbook
#+NAME guix-netbook
#+BEGIN_SRC scheme :tangle netbook.scm
(use-modules
 (gnu)
 (gnu services)
 (gnu services linux)
...
(operating-system
  ...
  (services
   (append (list (service xfce-desktop-service-type)
           ...
           ;; Remove some services that don't make sense
           (remove (lambda (service)
                     (let ((type (service-kind service)))
                       ;(or
                        (memq type
...
*** services
**** ci
#+NAME: guix-os-snips-ci
#+BEGIN_SRC scheme :tangle no
                   (service cuirass-service-type
                            (cuirass-configuration (specifications #~(list (specification
                                                                            (name "my-channel")
                                                                            (build (build '(channels my-channel));'(custom (jam ci)));
...
#+BEGIN_SRC scheme :tangle no
  ...
  (define nvidia-service-type
    (service-type
     ...
     (extensions
      (list
       (service-extension linux-loadable-module-service-type ;; add kernel module
                                 (const (list nvidia-driver)))
       (service-extension kernel-module-loader-service-type ;; load kernel module
                                 (const '("ipmi_devintf" "nvidia" "nvidia_modeset" "nvidia_uvm")))
       (service-extension udev-service-type (const (list nvidia-driver))))) ;; add udev rule
     (default-value '())
     (description "Install nvidia kernel module, service and udev rule")))

  (define nvidia-transform (options->transformation '((with-graft . "mesa=nvda"))))

  (define %nvidia-xorg-config ;; nvidia xorg configuration
    "Section \"Device\"
	Identifier     \"Device0\"
	Driver         \"nvidia\"
    ...
    EndSection")
    ;(append %default-kernel-arguments ("modprobe.blacklist=nouveau"))
    ;(xorg-configuration
    ;  (xorg-configuration
    ;    (modules (cons* nvidia-driver ;; nvidia-xorg
    ;        %default-xorg-modules))
    ;    (server (nvidia-transform xorg-server)) ; mesa to nvda graft
	;    (drivers '("nvidia"))
...
#+END_SRC
*** filesystem
#+NAME: guix-os-snips-zfs
#+BEGIN_SRC scheme :tangle no
  (use-modules (jam packages zfs))
  (use-package-modules linux file-systems admin)
...
#+END_SRC
*** kernel and sysctl harden
#+NAME: guix-os-snips-kicksecure
#+BEGIN_SRC scheme :tangle no
...
#+NAME: cuirass-jobs
#+BEGIN_SRC scheme :tangle no
; WIP This does not work as guix git cannot find module during profile build
...
(define (tarball-jobs store system)
  ...
  (list
   (->job "binary-tarball-guile"
          (run-with-store store
            (mbegin %store-monad
              (set-guile-for-build (default-guile))
              (>>= (profile-derivation (packages->manifest (list default-guile)))
                   (lambda (profile)
                     (self-contained-tarball "default-guile-binary" profile
                                             #:profile-name "default-guile"
                                             ;#:localstatedir? #t
                                             #:compressor (lookup-compressor "xz")))))
            #:system system))))

(define (cuirass-jobs-tar store arguments)
  "Register Cuirass jobs."
  (tarball-jobs store (arguments->systems arguments)))
