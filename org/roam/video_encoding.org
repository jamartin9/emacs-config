...
#!/usr/bin/env python3
#awsmfun dynamic tonemap similar function when using options for dovi (includes chroma)
import awsmfunc
#pb_opts = awsmfunc.types.placebo.PlaceboTonemapOpts(dst_max=130, dst_min=0) # 130 nits
...
src = core.ffms2.Source("./source-ng.mkv")
clip = core.std.Trim(src, first=1000, last=1015) # test frames
clip = core.resize.Spline36(clip, format=vs.YUV444P16) # 16 bit depth
#clip = vsutil.depth(clip, 16)
tonemapped_clip = core.placebo.Tonemap(clip=clip, src_csp=1, dst_csp=0, use_dovi=True) # map dst(bt709)
tonemapped_clip = core.resize.Spline36(tonemapped_clip,
                                       format=vs.YUV444P16,
                                       matrix_s="709",
                                       width=1280,
                                       height=720)
final_clip = vsutil.depth(tonemapped_clip, 8, dither_type="error_diffusion")
...
# example encode 10 bit HDR with x265
...
Dolby Vision is 12 bit with high dynamic range and metadata
...
Lossy AC3 (DolbyDigital and E-AC3 support more channels/bitrate) for <= 720p, uses dialnorm tho that lowers db range compared to DTS
...
** HEVC/AVC
*** x265
x265 --crf 18 --preset slower --profile main10 --output-depth 10 --rc-lookahead 60 --ctu 32 --frame-threads 1 --no-sao --deblock -2:-2 --qcomp 0.65 --aq-mode 3 --ref 6 --bframes 8
x265 --y4m - --preset slow --level-idc 5.1 --high-tier --input-depth 10 --output-depth 10 --rd 4 --ref 5 --psy-rd 2 --psy-rdoq 1 --no-rect --no-amp --aq-mode 4 --no-hevc-aq --aq-strength 0.9 --ctu 64 --ipratio 1.3 --pbratio 1.2 --no-cutree --rc-lookahead 40 --subme 5 --me 3 --merange 57 --bframes 12 --qcomp 0.6 --master-display "G(8500,39850)B(6550,2300)R(35400,14600)WP(15635,16450)L(10000000,20)" --colorprim bt2020 --colormatrix bt2020nc --transfer smpte2084 --range limited --max-cll "729,159" --vbv-bufsize 160000 --vbv-maxrate 160000 --hdr10 --hdr10-opt --repeat-headers --hrd --aud --deblock -3:-3 --no-sao --selective-sao 0 --strong-intra-smoothing --rskip 0 --crf 15 --limit-modes --limit-refs 1 --limit-tu 4 --max-merge 4 --tu-intra-depth 4 --tu-inter-depth 4 --b-intra --weightb --no-pmode --chromaloc 2 --no-frame-dup --dolby-vision-profile 8.1 --dolby-vision-rpu "RPU_modified.bin" --y4m --output Testing.hevc -- 2> Testing.log
changes to above args: --ref 6 --max-merge 5 --psy-rdoq 2 --ipratio 1.4 --pbratio 1.3 --aq-mode 3 --aq-strength 0.85 --qcomp 0.65
aq-mode 2 aq-strength 1
crf 16 - 18 for quality, crf 20-22 for speed
