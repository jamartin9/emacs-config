...
** Large Language Models
  ...
  - [[https://github.com/ggerganov/llama.cpp][llama.cpp gguf is 4 bit and cpu]] (adding more gpu and 3/5/6 bit quant)
    - ik_llama.cpp fork with cpu/quant improvements
      - fused deltanet for qwen3.5
        - ~-fdn 65536~ fuse deltanet when smaller than ubatch
        - 2782.81 MiB KV cache with q8 and 2142.81 with q6
        - fused deltanet3 (patched for f32 attention to stop rambling)
        - segfaults at warmup image using ~--mmproj ../qwen3.5-35B-A3B-mmproj-F32.gguf~
        - ~../llama-server-ik --model ../Qwen3.5-35B-A3B-IQ4_XS.gguf --ctx-size 262144 --temp 0.6 --top-p 0.95 --top-k 20 --min-p 0.00 --threads 12 --cache-type-k q6_0 --cache-type-v q6_0 -fa on -ger -khad --cache-ram 0 -fdn 65536 --threads-http 1 --no-warmup --jinja~
        - ~-mqkv -rtr~ disable mmap but gives 0.5 tk/s more
          - cpu mmap 16 tk/s 16.5tk/s with mkqv rtr w/fused deltanet q6 cache
      - flash mla ~-mla 3~ supported for cpu offloading on quants Q6 and Q8 for deepseek or glm4.7 lesser quants need ~-DGGML_IQK_FA_ALL_QUANTS=ON~ for cmake
        - ~../llama-server-ik --model ../GLM-4.7-Flash-REAP-23B-A3B-IQ4_NL.gguf --temp 0.7 --top-k 50 --top-p 1.00 --min-p 0.01 --dry-multiplier 1.1 --threads 12 --jinja -mla 3 -fa on -khad -amb 256 --cache-ram 0 -mqkv -rtr -ger --cache-type-k q8_0 --cache-type-v q8_0~
        ...
        - cpu build ~cmake -B build -DGGML_CUDA=OFF -DCMAKE_CUDA_FLAGS="-Wno-deprecated-gpu-targets" -DCMAKE_CUDA_HOST_COMPILER=g++-14 -DGGML_IQK_FA_ALL_QUANTS=ON && cmake --build build --config Release -j 8~
