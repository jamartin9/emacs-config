...
** Large Language Models
  ...
  - [[https://github.com/ggerganov/llama.cpp][llama.cpp gguf is 4 bit and cpu]] (adding more gpu and 3/5/6 bit quant)
    ...
    - ik_llama.cpp fork with cpu/quant improvements
      - ~-mtp~ ~--draft-max 3 --draft-p-min 0.75~ for mtp speculative decoding
        - model must have either a drafter or MTP tensors preserved
        - ~-mtp --draft-max 1 --draft-p-min 0.0~ for current qwen3.6 dense mtp support
        - Requant qwent mtp layer during runtime with ~-mtprot iq4_ks~ (better perf but costs ~0.6G mem)
        - gemma4 assistant drafter MTP with ~--spec-type mtp -md ../gemma-4-26B-A4B-it-assistant-Q8_0.gguf --draft-max 3 --draft-p-min 0.0~
      ...
      - fused deltanet for qwen3.5 (35B-A3B)
        ...
        - ~../llama-server-ik --model ../Qwen3.5-35B-A3B-IQ4_XS.gguf --mmproj ../qwen3.5-35B-A3B-mmproj-F32.gguf --no-warmup --ctx-size 262144 --temp 0.6 --top-p 0.95 --top-k 20 --min-p 0.00 --repeat-penalty 1.0 --threads 12 --cache-type-k q6_0 --cache-type-v q6_0 -fa on -ger -khad -vhad -muge -rtr --cache-ram 0 --ctx-checkpoints 0 --ctx-checkpoints-tolerance 0 --no-context-shift --threads-http 1 --no-warmup --jinja~
