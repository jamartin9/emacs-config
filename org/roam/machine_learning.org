...
* Machine learning
** notes
- turboquant 2/3/4 bit for kv cache added by google improves quant by coordinate translation and rotation(reduces irregularity)
...
- gemma scope is a sae for gemma feature exploration https://hf.co/google/gemma-scope
  - gemma scope 2 https://hf.co/google/gemma-scope-2
- qwen scope is a sae for qwen3.5 dense and moe https://hf.co/spaces/Qwen/QwenScope
...
- transformers context length prediction can be expanded from the size used in after mid training during the long-context extension stage with long documents
...
** Large Language Models
  - mistral4 small uses MLA like deepseek, glm4.7-flash etc
  - nvidia nemotron nano models use ssm/mamba hybrid layers
    - added latent attention
  ...
  - vllm can serve openai api locally
    ...
    - build custom docker image for cpu only avx2 (bf16,f32,f16 only) using ~--build-arg VLLM_CPU_DISABLE_AVX512=true~
      - cpu docker images now available ~podman run  -v /gnu/git/llms/scripts/vllm:/root/.cache/vllm:z -v /gnu/git/llms/huggingface:/root/.cache/huggingface:z -e "HF_TOKEN=$HF_TOKEN" -e "VLLM_LOGGING_LEVEL=DEBUG" -e "ON_CPU=1" -e "VLLM_CPU_X86=true" -e "ONEDNN_MAX_CPU_ISA=AVX2" -e "VLLM_TARGET_DEVICE=cpu" -e "VLLM_CPU_KVCACHE_SPACE=2" -p 8000:8000 --ipc=host --rm vllm/vllm-openai-cpu:v0.20.1-x86_64 --model google/gemma-4-E2B-it --max-model-len 1024 --language-model-only~
  ...
  - [[https://github.com/ggerganov/llama.cpp][llama.cpp gguf is 4 bit and cpu]] (adding more gpu and 3/5/6 bit quant)
    - llama.cpp mtp ~--spec-type draft-mtp --spec-draft-n-max 6~ for qwen3.6 MTP
      - perf was still lower than ik_llama w/o MTP
    - ik_llama.cpp fork with cpu/quant improvements
      - add ~--parallel-tool-calls~
      - webui and mcp proxy with ~--webui llamacpp --webui-mcp-proxy~
      - multiple specs with ~--spec-type ngram-mod:n_max=64,n_min=2,ngram_size_n=8 --spec-type mtp:n_max=1,p_min=0.0~
        - model must have either a drafter or MTP tensors preserved
        - Requant qwent mtp layer during runtime with ~-mtprot iq4_ks~ (better perf as reduced vocab size in output tensor but costs ~0.6G mem)
        - gemma4 assistant drafter MTP with ~-md ../gemma-4-26B-A4B-it-assistant-Q8_0.gguf~
          - ~-ngld 99~ for drafter gpu offload
          - quant the kv of the draft model ~-ctkd~ and ~-ctvd~
      - ~-c 524288 --rope-scaling yarn --rope-scale 2 --yarn-orig-ctx 262144~ for yarn scaling to larger context for qwen3.6
      - ~--fit~ and ~--fit-margin~ for gpu tensor offload
      - ~--smart-expert-reduction~ is basically REAP
      - supports vision for some models
      - gemma4
        - ~llama-server-ik --model ../google_gemma-4-26B-A4B-it-IQ4_NL.gguf --ctx-size 32768 --temp 1.0 --top-p 0.95 --top-k 64 --min-p 0.0 --repeat-penalty 1.0 --cache-type-k q6_0 --cache-type-v q6_0 -khad -vhad --threads 12 -fa on --cache-ram 0 --no-context-shift --no-warmup -ger -muge -rtr -mqkv --threads-http 1 --jinja --chat-template-kwargs '{"enable_thinking":true}'~
          - use ~--swa-full~ with llama.cpp for comparable SWA. llama.cpp supports iSWA(checkpointing, compression, fixed size instead of growing with context)
            - llama.cpp has better perf for cpu only
      - fused deltanet for qwen3.5 (35B-A3B)
        - qwen 3.6 improved on qwen 3.5
          - drift in ssm_conv1d tensors for qwen 3.6 corrected with Wasserstein and Qwen-Plus quants?
        - ~-khad~ or ~-vhad~ needs power of 2 attention head size (no MLA)
          - Hadamard transforms rotates the K or V cache for better quantized accuracy
          - only quant kv to q8 without Hadamard for accuracy
        - ~-muge~ merged gate/up/down ~ffn_down_exps~, ~ffn_gate_exps~, ~ffn_up_exps~ requires the same attention type
          -  unsloth ~Q4_K_L~ does match but the IQ quants do not match and cause assertion error in ik_llama
          - bartowski quants match as do ubergarms (and AesSedia?)
        ...
        - 2782.81 MiB KV cache with q8 and 2142.81 with q6
          - K quant is more important than V quant
        - ~../llama-server-ik --model ../Qwen3.6-35B-A3B-IQ4_XS.gguf --mmproj ../qwen3.6-35B-A3B-mmproj-F32.gguf -amb 256 --cpu-moe --no-kv-offload --no-warmup --ctx-size 262144 --temp 0.6 --top-p 0.95 --top-k 20 --min-p 0.00 --repeat-penalty 1.0 --threads 12 --cache-type-k q8_0 --cache-type-v q8_0 -fa on -ger -khad -vhad -muge -rtr --cache-ram 0 --no-context-shift --threads-http 1 --no-warmup --jinja~
        - ~-mqkv -rtr~ disable mmap but gives tk/s more
      - flash mla ~-mla 3~ supported for cpu offloading on quants Q6 and Q8 for deepseek or glm4.7 lesser quants need ~-DGGML_IQK_FA_ALL_QUANTS=ON~ for cmake
        - ~../llama-server-ik --model ../GLM-4.7-Flash-REAP-23B-A3B-IQ4_NL.gguf --temp 0.7 --top-k 50 --top-p 1.00 --min-p 0.01 --dry-multiplier 1.1 --threads 12 --jinja -mla 3 -fa on -amb 256 -muge -mqkv -rtr -ger --cache-type-k q8_0 --cache-type-v q8_0 --cache-ram 0~
        - 16 tk/s w/16 threads on iq4 and 11 tk/s with Q6
          - oversubscription can sometimes give perf when not memory bottlenecked
        - context 200k(202752 is 5561.79 MiB), 32k is 898.88 MiB of KV cache
        - khad (Hadamard transforms for K-cache) with quantized cache (special q8_KV for the cache-type-K causes assert error?)
        ...
        - mqkv for merged matrix. disables mmap
        - rtr for runtime repack. disables mmap
        - ger for GROUPED_TOPK
        - muge for merged projections. disables mmap
        - fmoe defaults to on
        - multiple of 4 for threads for simd(gh issue mentioned this?)
        - cpu build ~cmake -B build -DGGML_CUDA=OFF -DGGML_IQK_FA_ALL_QUANTS=ON && cmake --build build --config RelWithDebInfo -j 8 --clean-first~
...
#!/usr/bin/env bash
# https://github.com/ikawrakow/ik_llama.cpp/issues/1471
custom="
...
blk\.[0-4]\.ffn_down_exps\.weight=iq5_ks
...
#    fastapi
#    torchao
#    optimum-intel[openvino,nncf] @ git+https://github.com/huggingface/optimum-intel.git" # doesnt support qwen3.5 yet or transformers 5 (also needed for qwen)
#    openvino>=2026.0.0
#    nncf>=2.14.0
#    uvicorn
#    ninja>=1.13.0 # for build native
...
# serving qwen3.5 0.8B
#!/usr/bin/env python
import os
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, AutoConfig, TorchAoConfig #BitsAndBytesConfig
from fastapi import FastAPI
import uvicorn
from time import perf_counter
#import openvino
#import nncf
#from optimum.intel import OVModelForCausalLM, OVWeightQuantizationConfig # doesnt support qwen 3.5 or transformers 5 yet...
import torchao
from torchao.quantization import Int8DynamicActivationIntxWeightConfig, Int8DynamicActivationInt8WeightConfig, PerRow # quantize_ QAT supported Int4WeightOnlyConfig failed with cpu (hqq and gemm backends)
#from torchao.quantization.qat import QATConfig # wrap config with QATConfig(quant_config) for 'fake quant' before and after training

#lib_path = "/gnu/git/llms/scripts/pyserv/ao/torchao/_C.abi3.so" # will throw library not loaded error when kernels are not found...
#try:
#    torch.ops.load_library(lib_path)
#    print("✅ Success: Library loaded manually!")
#except Exception as e:
#    print(f"❌ Failed to load: {e}")
#torch.ops.torchao._pack_8bit_act_4bit_weight # is the check in torchao and seems to be arm only?

os.environ["TORCHINDUCTOR_CACHE_DIR"] = "/gnu/git/llms/scripts/pyserv/kernel_cache"
torch._inductor.config.fx_graph_cache = True

model_id = "Qwen/Qwen3.5-0.8B"
tokenizer = AutoTokenizer.from_pretrained(model_id)
#bnb_quant_config = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_use_double_quant=True, bnb_4bit_compute_dtype=torch.float32)
#vino_quant_config  = OVWeightQuantizationConfig(bits=4) # bits=8
torchao_quant_config = Int8DynamicActivationInt8WeightConfig(set_inductor_config=True, version=2)#weight_only_decode=True,  #Int8DynamicActivationIntxWeightConfig(weight_dtype=torch.int4) # use the da8w4 kernel path # intx_packing_format="opaque_torchao_auto" lowbit is arm only # QAT only support int8dynamicintx with weight int4 activation int8, cpu TorchAO kernel library is not loaded and uses lowbit w/o arm for kledia kernels
tochao_quantization_config = TorchAoConfig(quant_type=torchao_quant_config)

model = AutoModelForCausalLM.from_pretrained( #OVModelForCausalLM.from_pretrained(
    model_id,
    dtype=torch.float32, # torch_dtype is deprecated but dtype throws error when saving some configs...
#    quantization_config=tochao_quantization_config,#vino_quant_config,#bnb_quant_config,
    device_map="cpu")
model = torch.compile(model, #mode="max-autotune",
                      fullgraph=False#, backend="openvino", options={"device": "CPU", "model_caching": True, "cache_dir": "/gnu/git/llms/scripts/pyserv/vino", "config":{ "PERFORMANCE_HINT": "LATENCY", "INFERENCE_PRECISION_HINT": "f32"}} # fullgraph off so it doesn't silently fail on gaps
                      )
print("Checking kernel cache...")
with torch.no_grad():
    input_ids = torch.ones((1, 128), dtype=torch.long)
    model(input_ids)# generate doesn't compile?

#output_dir = "/gnu/git/llms/scripts/pyserv/qwen3.5-0.8B-torchao-int8-dynamic-8bit-cpuv2" # change name when quant changes
#model.save_pretrained(output_dir) # save and reload models for weight only config?
...
async def chat(prompt: str):
    ...
    with torch.no_grad():
        ...
        output = model.generate(**inputs,
                                ...
                                return_dict_in_generate=True) # dict for perf measurement
        stop = perf_counter()
        gen_tokens = output.sequences[0][inputs.input_ids.shape[1]:]
        tks = len(gen_tokens) / (stop - start)
        print("tk/s: " + str(tks)) # dont count decode only gen
    return {"response": tokenizer.decode(output[0], skip_special_tokens=True)}
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8000)
#curl "127.0.0.1:8000/chat?prompt=What%20is%20the%20capital%20of%20France?" -XPOST
# f32: just under ~9tk/s with vino or inductor or plain no compile. ik_llama gives ~14tk/s with same F32
# quantized: bnb gives 0.5 tk/s and torachao Int8DynamicActivationInt8WeightConfig() gives 3.5 tk/s, 1.5 with weight_only v2 8 tk/s plain. Int8DynamicActivationIntxWeightConfig 4bit weight gives ~2 tk/s. Int4WeightOnly gives does not support. Vino does not support yet.
# for tochao maybe add BUILD_TORCHAO_EXPERIMENTAL=1 but seems cpu inference is arm/mac only - UV_LINK_MODE=copy USE_CPP=1 USE_CUDA=0 USE_CPU_KERNELS=1 uv pip install --no-build-isolation -v --no-binary torchao -e ./ao 2>&1 | tee -a output.log
# LD_LIBRARY_PATH=/gnu/git/llms/scripts/pyserv/.venv/lib/python3.12/site-packages/torch/lib
