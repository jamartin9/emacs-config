...
- change .net GC to Server in ~/etc/default/jellyfin~ ie ~COMPlus_gcServer=1~ for lower cpu but more ram usage
  - possible with env var in .net 10+ (10.11.0 is .net 9)
  - set ~DOTNET_GCConserveMemory=9~ for minimum memory usage but more runs
...
- roku client
  - disable direct playback and set bitrate limit (default 10 mbps h264 roku) to force transcode on errors. set force transcode with remux disabled for compat
- web client
...
- config
  ...
  - set subtitles from web user page to None or Forced and relog on each client to stop default play
- running jellyfin with podman container
  ...
  - rshared/rbind opts for recursive mounts
    - rslave with [[id:308edd9a-90d8-44f3-ab1f-fae4942b0eae][rarfs]] using a container for volume mount propagation
...
#+BEGIN_SRC shell :tangle no
  ...
  --volume /home/jam/rarfs:/media2:rslave \
...
podman run --rm -it --device nvidia.com/gpu=all --security-opt=label=disable --env NVIDIA_DRIVER_CAPABILITIES=all --env NVIDIA_VISIBLE_DEVICES=all -p 8096:8096 \
       ...
       -v /gnu/git/torrents/videos/:/media3:rslave \
       -v /storage/Videos/:/media3/Videos \
       -v /gnu/git/torrents/shows/:/media5 \
