...
* Github Stars
** propensive/soundness
  - Soundness is an ecosystem of libraries for writing direct-style code with Scala 3
** getkyo/kyo
  - Kyo introduces a novel approach based on algebraic effects to deliver straightforward APIs in the pure Functional Programming paradigm in Scala (JS, JVM and Native)
** cbftp/cbftp-svn
  - Cbftp is an advanced multi-purpose FTP/FXP client that focuses on efficient large-scale data spreading, while also supporting most regular FTP/FXP use cases in a modern way. It runs in a terminal and provides a semi-graphical user interface through ncurses
** johnmn3/injest
  - +>, +>>, x>>, =>>: Auto-transducifying, auto-parallelizing path thread macros for clojure performance
** VirtusLab/sandcat
...
** cmu-db/optd
  - Query Optimizer Service for datafusion
** hectorvent/floci
  - Light, fluffy, and always free - AWS Local Emulator. Foss localstack
...
** level1techs/siomon
  - A comprehensive Linux hardware information and real-time sensor monitoring tool
** wizicer/anonymous_predicting_market
  - A Chia "dark" prediction market with encrypted bets using threshold encryption and zero-knowledge proofs, preventing copy-trading and front-running.
** ghostdogpr/purelogic
  - Direct-style pure domain logic for Scala. monad free
** Verilean/hesper
...
** codeberg.org/ezemtsov/ewm
  - Wayland compositor that runs inside Emacs. requires PGTK for native Wayland frame creation
** Open-Wine-Components/umu-launcher
  - Unified launcher for Windows games on Linux. Shared protonfixes. Essentially a copy of the Steam Runtime Tools and Steam Linux Runtime with some modifications for use outside of Steam.
** zio/zio-blocks
  - Powerful, joyful building blocks for modern cloud-native applications. Supports optic schema derivation and patch generation.
** mkirchner/hamt
  - A hash array-mapped trie implementation in C99. Uses tagged pointers for node type with murmur3 (32bit)
** nimtable/nimtable
  - The observability platform for Iceberg lakehouses. Optimization and compaction.
...
** LizardByte/Sunshine
  - Self-hosted game stream host for Moonlight. Self-hosted GeForce NOW alternative
** moonlight-stream/moonlight-qt
  - GameStream client for PCs (Windows, Mac, Linux, and Steam Link).
** edin-dal/scair
  - MLIR Scala Compiler Framework
** boonzy00/ringmpsc
  - Lock-free MPSC channel in Zig achieving 50+ billion messages/second via ring-decomposed architecture
** ZHZisZZ/dllm
  - dLLM: Simple Diffusion Language Modeling. Can convert AR to diffusion model.
** sraoss/pg_ivm
  - IVM (Incremental View Maintenance) implementation as a PostgreSQL extension
** gudzpoz/Juicemacs
  - An absolutely-not-solid Emacs implementation (WIP) using graal and truffle
...
** deuxfleurs-org/garage
  - S3-compatible object store for small self-hosted geo-distributed deployments. Main repo: https://git.deuxfleurs.fr/Deuxfleurs/garage
** rustfs/rustfs
...
** micytao/vllm-playground
  - web interface for managing and interacting with vLLM servers
** hirrolot/mazeppa
  - Mazeppa is a modern supercompiler intended to be a compilation target for call-by-value functional languages to c11
** lakehq/sail
  - Rust based Compatible with the Spark Connect protocol, supporting the Spark SQL and DataFrame API compute engine.
** runestubbe/Crinkler
  - Crinkler is an executable file compressor (or rather, a compressing linker) for compressing small 32-bit Windows demoscene executables
** superfly/corrosion
  - Gossip-based service discovery (and more) for large distributed systems.
** future-file-format/F3
  - data file format via embedded Wasm decoders
** bearcove/arborium
  - An opinionated tree-sitter + tree-sitter-highlight + grammars bundle
** archwisp/OBD2Proxy
  - OBD-II Proxy and Emulator
** fenugrec/freediag
  - OBD2 scantool
** cedricp/ddt4all
  - OBD tool (fiat?)
...
** osmus/layercake
  - Thematic extracts of OpenStreetMap data in geoparquet and flatgeobuf formats
** ulissesf/qmassa
  - Rust terminal-based tool for displaying GPUs usage stats on Linux.
** facebook/openzl
  - takes a description of your data and builds from it a specialized compressor optimized for your specific format
** yaledb/calvin
  - Calvin is a scalable transactional database system that leverages determinism to guarantee active replication and full ACID-compliance of distributed transactions without two-phase commit. Most of the code is for VLDB 2014 paper: An Evaluation of the Advantages and Disadvantages of Deterministic Database System.
** mwette/nyacc
  - parser generator, C parser and FFI helper for Guile with SXML. c99/javascript/julia/python/java
** webyrd/Barliman
  - propagator network for smart editor that performs program systhesis
** miniupnp/miniupnp
...
** lukefleed/compressed-intvec
  - fixedVec for fixed-width encoding, O(1) mutable and atomic access and IntVec for variable-length.
** build-trust/ockam
...
** continue-revolution/sd-webui-segment-anything
  - Segment Anything for Stable Diffusion WebUI with grounding DINO
...
** apache/datafusion
  - Apache DataFusion SQL Query Engine in Rust. Can add custom metadata for optimized scans
...
** Barre/ZeroFS
  - ZeroFS - The Filesystem That Makes S3 your Primary Storage. ZeroFS is 9P/NFS/NBD on top of S3. With zfs
** bearcove/fluke
...
** ChemAI-Lab/molpipx
  - std::autodiff for a differentiable version of Permutationally Invariant Polynomial (PIP) models in JAX and Rust
...
** city96/ComfyUI-GGUF
  - GGUF Quantization support for native ComfyUI models (flux)
** cjdelisle/cjdns
...
** clockworklabs/SpacetimeDB
  - ECS database and server combined into one.
** clojure/clojure
...
** comaps/comaps
  - The main code repository of CoMaps, a community-led fork of Organic Maps. Reinforced with commitment to transparency, privacy and being not-for-profit. https://www.comaps.app/
...
** containers/ramalama
  - The goal of RamaLama is to make working with AI boring. Attempts to upstream changes unlike Ollama
...
** dottxt-ai/outlines
  - Structured Text Generation output to json, grammar, choice etc
...
** duckdb/ducklake
  - DuckLake is an integrated data lake and catalog format that uses a database to manage metadata with less round trips than iceberg/delta lake
** ebfull/halo
  - ZK-SNARK setup
...
** frida/frida
  - Clone this repo to build Frida. reverse engineering and api hooking in javascript
...
** geohot/tinygrad
  - You like pytorch? You like micrograd? You love tinygrad! https://stats.tinygrad.win/
** geoserver/geoserver
...
** gitlab.gnome.org/GNOME/sysprof
  - sampling profiler using a kernel module to generate stacktraces for userland sysprof.
** git-lfs/git-lfs
...
** huggingface/trl
  - Train transformer language models with reinforcement learning. Can use vllm for fast inference.
** hvmi/hvmi
...
** ispc/ispc
  - Intel Implicit SPMD Program Compiler for a variant of the C programming language with SIMD
** iv-org/invidious
...
** john-parton/slatedb-nbd
  - NBD binding over SlateDB with minimal abstractions.(zfs on block device backed by object storage)
...
** LeetArxiv/Finite-Field-Assembly
  - The Finite Field Assembly Programming Language. designed to emulate GPUs on regular CPUs using Pure Mathematics.
** lerna/lerna
...
** pythonlover02/Proton-Sarek
  - Compatibility tool for Steam Play based on Wine and additional components. For Older Gpus or No vulkan support
** python/mypy
...
** rcardin/yaes
  - An experimental effect system in Scala using capability passing style
** reactos/reactos
...
** simplex-chat/simplex-chat
  - SimpleX - the first messaging network operating without user identifiers of any kind - 100% private by design! iOS, Android and desktop apps. Dead from adding crypto bs - https://x.com/SimpleXChat/status/1983849427640193293
...
** souffle-lang/souffle
  - Soufflé is a variant of Datalog for tool designers crafting analyses in Horn clauses. Soufflé synthesizes a native parallel C++ program from a logic specification using futamura projections.
...
** tanelpoder/0xtools
  - 0x.Tools: X-Ray vision for Linux systems. BPF like top with traces. xtop, xstack etc.
** tcdi/pgx
...
** topling/toplingdb
  - ToplingDB is a cloud native LSM Key-Value Store with searchable compression algo and distributed compaction. Fork of RocksDB
** torproject/stem
...
** tsotchke/quantum_geometric_tensor
...
** web-seven/overlock
  - CLI tool designed to simplify the management of Crossplane resources. supports KinD, K3s, and K3d clusters, making it ideal for local development and testing of Crossplane configurations, providers and functions
...
** eunomia-bpf/bpftime
  - Userspace eBPF runtime for Observability, Network, GPU & General Extensions Framework (sm_60+)
** yandex/perforator
...
* machine learning
** taco-project/FlexKV
  - A KVCache Manager for High-Performance Distributed Inference
** ai-dynamo/aiperf
  - AIPerf is a comprehensive benchmarking tool that measures the performance of generative AI models served by your preferred inference solution.
** lampepfl/TACIT
  - TACIT — Tracked Agent Capabilities In Types for Scala
** llm4s/rag_in_a_box
  - LLM4S RAG in a box. REST API for document ingestion and querying
** shap/shap
...
** arcee-ai/DistillKit
  - An Open Source Toolkit For LLM Distillation. Supports offline, online and moes.
** THUDM/slime
...
** normal-computing/extended-mind-transformers
  - decoder-only transformer that retrieve and attend to an external cache of key-value pairs without finetuning
** comfyanonymous/ComfyUI
  - A powerful and modular stable diffusion GUI with a graph/nodes interface. (Stable Diffusion XL)
** dmlc/xgboost
  - Scalable, Portable and Distributed Gradient Boosting (GBDT, GBRT or GBM) Library, for Python, R, Java, Scala, C++ and more. Runs on single machine, Hadoop, Spark, Dask, Flink and DataFlow. For tabular regression decision tree data.
...
** ggerganov/llama.cpp
  - Port of Facebook's LLaMA model in C/C++. llama-run to run models. see fork ikawrakow/ik_llama.cpp for more quant formats/optimizations
...
** SylphAI-Inc/AdalFlow
  - AdalFlow: The library to build & auto-optimize LLM applications with Auto diff prompts
** vgel/repeng
...
** hotdata-dev/datafusion-ducklake
  - A Rust-native DuckLake engine built on Apache DataFusion
** datafusion-contrib/datafusion-federation
  - Allow DataFusion to resolve queries across remote query engines pushing down as much compute as possible down.
** microsoft/RustTraining
  - Beginner, advanced, expert level Rust training material
** Palkovsky/stackaroo
  - Swap out of the OS-provided stack with your own custom stack (e.g. heap-backed).
** rex-rs/rex
...
** CeleritasCelery/rune
  - Rust VM for Emacs
** kanidm/kanidm
  - A simple, secure, and fast identity management platform in Rust
** rust-netlink/netlink-proto
  - asynchronous implementation of the netlink protocol
** helsing-ai/sguaba
  - rigid body transforms (aka "spatial math")
** davidlattimore/wild
  - A very fast linker for Linux
** marvinborner/optimal-effects
  - lambda calculus interaction net eval
** almogdepaz/veil
  - Zero-knowledge proof system for running Chialisp programs privately
** bootreer/vroom
...
** spiraldb/fastlanes
  - FastLanes compression Library rust port for SIMD bitpacking
** ashvardanian/StringZilla
  - fast string search, hashing, sorting, edit distances, sketches, and memory ops
** tracel-ai/cubecl
  - Multi-platform high-performance compute language extension for Rust compute kernels.
** AeneasVerif/aeneas
...
** BurntSushi/aho-corasick
  - A fast implementation of Aho-Corasick in Rust for finding occurrences of many patterns at once with SIMD.
** BurntSushi/fst
...
** fortress-build/whirlwind
  - fully asynchronous, sharded hashmap for Rust.
** foyer-rs/foyer
...
** Hanaasagi/tokio-splice
  - splice(2) based `copy_bidirectional` in tokio
** hermit-os/hermit-rs
...
** iex-rs/iex
  - Faster Idiomatic exceptions for Rust
** iomesh/libuzfs-rs
...
** japaric/cargo-call-stack
  - Whole program static stack analysis for stack overflow
** JelteF/derive_more
...
** pacak/cargo-show-asm
  - cargo subcommand showing the assembly, llvm-mca, LLVM-IR and MIR generated for Rust code
** panamax-rs/panamax
...
** sarah-quinones/pulp
  - safe abstraction over SIMD instructions
** sbstp/rust-igd
  - Internet Gateway Device (UPNP) client
** sched-ext/scx
  - sched_ext schedulers and tools like scx_rustland
** smoltcp-rs/smoltcp
  - a smol tcp/ip stack
** spacejam/tla-rust
...
** sunfishcode/eyra
  - Rust programs written entirely in Rust
** tansu-io/tansu
  - Apache Kafka compatible broker with S3, PostgreSQL and Apache Iceberg.
** tauri-apps/tauri
...
* utils
** mikesart/gpuvis
  - trace-cmd gpu visualizer for intel/amd/mesa/steamvr
** keylase/nvidia-patch
  - This patch removes restriction on maximum number of simultaneous NVENC video encoding sessions imposed by Nvidia to consumer-grade GPUs.
** codeberg.org/tazjin/reka
  - window manager inside of Emacs pgtk for Wayland
** andikleen/pmu-tools
  - Intel PMU profiling tools
** andreas-abel/uiCA
  - uops.info Code Analyzer for intel bottleneck performance analysis
** btcpayserver/btcpayserver
  - Accept Bitcoin payments. Free, open-source & self-hosted, Bitcoin payment processor. Plugins for monero and chia.
** BurntSushi/ripgrep
...
** gitlab.torproject.org/tpo/core/oniux
  - isolate arbitrary application over the Tor network
** gpg/gnupg
