...
- c++20
  - modules, concepts(templates constraints), ranges, coroutines
    - "As of C++20, there are no cases in which statically linked programs require virtual functions." https://www.youtube.com/watch?v=gTNJXVmuRRA , concepts bind interfaces(type classes), deduced class templates for object config, std::variant for runtime
      - most implementations are still just a jump table of pointers
- c++23
  - generators, import std;, std::expected
- c++ 26
  - reflection (string like templates)
  - std::exec (standard async runtime)
...
*** [[id:4dabbc8e-a162-4e85-9803-82ea93e8c61f][scala]]
** javascript
*** typescript
