...
(ignore-errors
  (let* ((searx-confdir (concat (file-name-as-directory (xdg-config-home)) "searx"))
    ...
    (if (not (or (file-exists-p searx-conf) (file-symlink-p searx-conf)))
        (make-symbolic-link (concat (file-name-directory (or load-file-name buffer-file-name)) "searx.yml") searx-conf 1))))
#+END_SRC
** Tor Proxy
** Secret Key
#+NAME: secretkey
