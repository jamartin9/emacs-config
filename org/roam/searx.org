:PROPERTIES:
:ID:       b4a6be9e-1cb6-416a-b6e0-392ee9a28734
:END:
#+title: searxng
...
* Searxng config
The ~secret_key~ is dynamically generated.
The http(s) proxies are changed to socks5 for tor.
** Link searxng
#+NAME: link-searx-el
#+BEGIN_SRC emacs-lisp :tangle no :shebang "\":\"; emacs -Q --script searx.sh -- $@ $args ; exit $? # -*- mode: emacs-lisp; lexical-binding: t; -*-"
(ignore-errors
  (let* ((searx-confdir (concat (file-name-as-directory (xdg-config-home)) "searxng"))
        (searx-conf (concat (file-name-as-directory searx-confdir) "searxng.yml")))
    ...
    (if (not (or (file-exists-p searx-conf) (file-symlink-p searx-conf)))
        (make-symbolic-link (concat (file-name-directory (or load-file-name buffer-file-name)) "searxng.yml") searx-conf 1))))
#+END_SRC
** Tor Proxy
** Secret Key
#+NAME: secretkey
...
** searxng.yml
#+NAME: searx.yml
#+BEGIN_SRC yaml :tangle searxng.yml :noweb yes
general:
