...
* chia
- https://github.com/AbandonedLand/CircuitSage for powershell scripts for circuitdao
- https://rewards.fireacademy.io/distributor/datalayer-minions for datalayer minions reward distributor stake for dig tokens
- 2.7.1+ needs ozone-platform set for electron gui in ~gui/package.json~ ex. ~electron . --ozone-platform=x11~ (electron version changed default to auto)
  - extraction of electron zip fails with newer nodejs. extract electron manually and create the path.txt file with dist ~printf "electron" > ./node_modles/electron/path.txt~
...
- [[https://mojonode.com/explorer?q=select%20date_trunc(%27week%27,%20gs.ts)%20as%20datetime,%20coalesce(max(reorgs),%200)%20as%20%22max%20seq%20block%20reorgs%20per%20week%22%20from%0A%20%20generate_series(%272022-03-10%2000:00:00%27::timestamp,%20%0A%20%20%272023-03-21%2023:55:00%27::timestamp,%20interval%20%271%20day%27)%20gs(ts)%20%0A%20%20left%20join%20%0A%20%20(%0Aselect%20min(dt)%20as%20dt,%20count(seq)%20as%20reorgs%20from%20(%0A%20%20select%20height,%20max(created_at)%20as%20dt,%0A%20%20%20%20case%20when%20height%20-%20row_number()%20over%20(order%20by%20height)%20=%20-1%20then%20height%20%20%0A%20%20%20%20else%20height%20-%20row_number()%20over%20(order%20by%20height)%20end%20as%20seq%20from%20block_records%20%0Awhere%20reverted=true%0Agroup%20by%20height%0A)%20as%20t%0Agroup%20by%20seq%0A%20%20%20having%20count(seq)%20%3E%200%0A%20%20%20)%20as%20b%20%0A%20%20%20%20%20%0A%20%20%20%20%20on%20date_trunc(%27week%27,%20b.dt)%20=%20date_trunc(%27week%27,%20gs.ts)%0A%20%20%0A%20group%20by%20date_trunc(%27week%27,%20gs.ts)%0Aorder%20by%20date_trunc(%27week%27,%20gs.ts)%20][mojo node sql for block reorg]]
  - dead
- space farmers pool runs no longer runs a community node
  - was a light farming alternative since flexpool shutdown
...
#+NAME: btcpayserver-podman-updated-deps-json
...
#+END_SRC
  ...
  - create plugin json
#+NAME: btcpayserver-podman-updated-json
#+BEGIN_SRC json :tangle no
{
...
#+END_SRC
#+NAME: btcpayserver-podman-updated
#+BEGIN_SRC yaml :tangle no
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  - Debug
    - set master key hex from greenfield api
      ...
      - curl post the key. add optional ~, "addresses": ["xchADDR"]~ to the config object to create an initial invoice (generation seems broken with tested random pubkey?)
        - reset bad config with ~podman exec -it CONTAINER_ID psql -U postgres -d btcpayserver~ then ~UPDATE "Stores" SET "DerivationStrategies" = "DerivationStrategies" - 'XCH-CHIA' WHERE "Id" = 'STORE_ID';~ replacing STORE_ID and CONTAINER_ID
#+NAME: btcpayserver-podman-curl-json
#+BEGIN_SRC json :tangle no
