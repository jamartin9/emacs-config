...
*** notes
- 2.3+ disable prefetch per dataset
...
- When expanding rebalancing is not done leaving potentially higher resilver times in the future increasing the chance of cascading failure.
  - zfs rewrite added for rebalancing datasets
...
- SPECIAL vdevs store metadata (good for ssd) but need redundancy as they can take the pool down
  - can function as ZIL offload like L2 in 2.4.0+
...
*** guix on zfs
- see https://codeberg.org/guix/guix/pulls/1917#issuecomment-6976089 for modern patch based on the script/notes below
  - https://codeberg.org/hako/Rosenthal/ with patch at ~modules/rosenthal/packages/patches~
    - built into kernel https://github.com/openzfs/zfs/issues/10450#issuecomment-643654436 for (initramfs issues)
    - UKI kernel with limine failed on deploy and vm image creation with qcow-gpt (grub-efi issues)
    - zfs filesystem entries hang boot with vm image and error on deploy even with 2nd hd with pool and datasets
...
#+END_SRC
*** wip
    - WIP to add L2 without parsing zpool/zdb can use ioctl, nvpair with ZFS_IOC_POOL_CONFIG for the cache device path or parse zpool iostat json output in zfs 2.3+
      - get arc r/w bytes from procfs iostat.
      - send | recv of unmounted datasets will not be shown despite having stats currently as it uses a different flow
      - ZFS_IOC_POOL_STATS does not contain the spa_iostats exposed by under the per pool proc directory iostats file for per pool arc
        - includes 'vs_bytes' index with the read/write total bytes for each vdev_stat_t returned
        - appends 'l2cache' on vdev_root
        - appends 'spares' on vdev_root (active spare?)
        - zvols?
      - https://github.com/robn/veneer does most of this already in dataset_list and pools. The per dataset arc usage counters from iostats and total L2 io still come from procfs.
      - libzfs_core python bindings are generated from ~include/libzfs/libzfs_core.h~ which only contains lzc_ioctl_fd (no bindings generated atm) and not the real lzc_ioctl in ~lib/libzfs_core/libzfs_core.c~
        - lzc_ioctl for the zfs_ioc_pool_stats(spa_get_stats/ZFS_IOC_POOL_STATS/* 0x5a05 */) is not exposed by pyzfs yet.
          - lzc_ioctl calls lzc_ioctl_fd but is static
          - call open on ~/dev/zfs~ and use that for the fd. Still requires mocking out ~zfs_cmd_t~
        - zfs_ioc_pool_configs for the spa_all_configs(ZFS_IOC_POOL_CONFIGS/* 0x5a04 */) to get all pools/devicepaths
          - NetBSD builds but isn't functional in bottom or sysinfo with netbsd 10.1 as there is statvfs and getmntinfo changes from 9 that havent been done in libc yet (wait for EOL of 9)
      - sysctl lacks netbsd support
        - has arc kstats but not objset ie no dataset io counters
