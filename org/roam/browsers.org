...
- ctrl-shift-R for force cache reload
  - does not always work for wasm files; use network devtools 'Disable Cache'
...
** chrome
- ~chrome://on-device-internals/~ for model directory/status for ~window.Summarizer.availability()~
    ...
    - ~brave://flags~
      ...
      - WebGPU flag ~--enable-features=Vulkan~
        - ~--enable-features=Vulkan~ (not compat with wayland/ozone)
          - https://chromium.googlesource.com/chromium/src/+/refs/heads/main/docs/gpu/vaapi.md#VaAPI-on-Linux-with-Vulkan suggests ~--enable-features=AcceleratedVideoEncoder,VaapiOnNvidiaGPUs,VaapiIgnoreDriverChecks,Vulkan,DefaultANGLEVulkan,VulkanFromANGL~
...
** tools
- profile webkit(mesa too) with sysprof
- web archive https://replayweb.page/
- [[https://wasm-test.libreoffice.org/][libreoffice wasm]]
  - replaced with https://github.com/allotropia/zetajs
    - https://zetaoffice.net/demos/web-office/
    - https://zetaoffice.net/demos/convertpdf/
    - https://zetaoffice.net/demos/letter-address-vuejs3/index.html
      - include as iframe
- https://directfile.irs.gov/
- https://uica.uops.info/
- https://cowasm.sh/ (python wasm)
- https://shell.duckdb.org
  - ~load sqlite; load ducklake;~
  - ~attach 'ducklake:sqlite:metadata.sqlite' as my_ducklake (DATA_PATH 'data_files/');~
  - ~use my_ducklake;~
    - inspect files with ~FROM glob('*')~
    - test data ~CREATE TABLE nl_train_stations AS FROM 'https://blobs.duckdb.org/nl_stations.csv';~
- https://parquet-viewer.xiangpeng.systems/ is hosted XiangpengHao/parquet-viewer
- https://coalton.app/ for browser coalton
- https://deepwiki.com/ for repo summary
  - add user/repo on url path to generate
  - https://sidekickdev.com/ generates agent markdown from deepwiki repos
** data
...
- https://hianime.to
  - shutdown 3/13/2026
    - check popular https://everythingmoe.com/
- https://animekai.to
  - https://animekai.cc
  - https://animekai.ac
  - https://anikai.to
  - hardsub sites
    - https://gogoanime.by/
      - https://anitaku.io/
    - animepahe.pw animepahe.com animepahe.org
- https://cs.rin.ru
