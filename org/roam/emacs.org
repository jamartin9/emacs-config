...
(use-package shell-maker ; melpa error?
  :after agent-shell
...
(use-package agent-shell
  ...
  :config
  (jam/set-rust-path) ; need codex-acp ; set CODEX_HOME so the defaults can use local
  (setq ; agent-shell-openai-codex-command with agent-shell-path-resolver-function #'agent-shell--resolve-devcontainer-path for container
   agent-shell-goose-authentication (agent-shell-make-goose-authentication :openai-api-key (lambda ()
                                                                                             (let ((gemini-key (jam/auth-display (car (auth-source-search :user "GEMINI_API_KEY")))))
                                                                                               (setenv "GEMINI_API_KEY" gemini-key)
                                                                                               (setenv "GOOGLE_API_KEY" gemini-key)
                                                                                               (setenv "OPENAI_API_KEY" ""))
                                                                                             ""))
   agent-shell-openai-authentication (agent-shell-openai-make-authentication :api-key (lambda () (setenv "GEMINI_API_KEY" (jam/auth-display (car (auth-source-search :user "GEMINI_API_KEY")))) "")))) ; use codex for gemini

(use-package geiser-hoot ; guile-hoot (v0.8 in guix is missing repl.js - https://codeberg.org/spritely/hoot/issues/845)
  ;:vc (:url "https://codeberg.org/spritely/geiser-hoot" :rev "08b7702e9d2d2bd377ea7fd89608f09e84d2f26a") ; install from source until release
  :config (setq geiser-hoot-scheme-local-dir (concat user-emacs-directory (file-name-as-directory ".local") (file-name-as-directory "cache") "geiser-hoot"))
  :commands (geiser-hoot)) ;geiser-hoot-load-path
...
(use-package nov
  ...
  :commands (nov-mode))
   ...
   ; mcp.el
  (defun extract-structuredContent (orig-fun &rest args)
    (let ((structured (plist-get args :structuredContent)))
      (message "structured is : %s" structured)
      (concat (apply orig-fun args) "\n" structured))); add on structured data for codex's structuredContent { "threadId": "" }
  (advice-add 'mcp--parse-tool-call-result :around #'extract-structuredContent)

;;;###autoload
(defun jam/mpv-play (url)
  "Run mpv in eat terminal. 9 is volume down; 0 is up. Requires url-handler-mode for default completion (use home and del)."
  (interactive (list (read-file-name (format "Args(https://somafm.com/vaporwaves.pls): ") nil nil nil "https://somafm.com/vaporwaves.pls")))
  (eat (concat "mpv -vo=sixel " url) t))

;;;###autoload
(defun jam/eshell () "Open new eshell" (interactive) (eshell t));  :bind (:map jam/open ("e" . jam/eshell))

; reka config with just emacs-pgtk in ~/.config/river/init
(module-load "/gnu/git/reka/target/release/libreka.so")
(load-file "/gnu/git/reka/lisp/reka.el")
(require 'reka)
(reka-enable)
