...
* Postgresql
** notes
- max_standby_streaming_delay with queries that conflict with replication can cause lag in hybrid olap/oltp workloads
- requires vaccums on row deletion to avoid page fragmentation. It appends to bottom unlike mysql/innodb
*** addons
