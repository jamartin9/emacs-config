...
*** Intel
- haswell added hardware lock elision (HLE) with intel TSX
  - disabled in microcode due to security issues with instructions added later for mitigations
  - QEMU has options for passthrough ~tsx-ctrl~
  - RSPCS3 used it for large framerate improvements
** M Series
  - 128 bit cache line(false sharing?)
** Backdoors
