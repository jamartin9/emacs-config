...
* podman
** podman notes
- see logs from containers that fail to start with ~podman start CONTAINER_ID -a~
- see exit code ~podman inspect btcpay-deployment-pod-btcpay --format '{{.State.ExitCode}}'~ ex 132 is illegal instruction
  - ~podman update --restart=no ID~ to prevent restarts
