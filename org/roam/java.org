...
- openjdk 26 added ~-Xlog:cpu~ and the ~MemoryMXBean.getTotalGcCpuTime()~ for measuring cpu overhead on heap sizes
- ZGC takes ~3% of heap
- ZGC emits jfr event ZAllocationStall
...
- C2 uses SuperWord paper for optimization via alignment with simd vectorization
- java adding type classes with witness and Monoid from 'Growing the Java Language #JVMLS' by Brian Goetz
